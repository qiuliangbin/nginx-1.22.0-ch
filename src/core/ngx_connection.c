...
/**
 * @description: 逐一设置cycle->listening数组每个元素
 * @param {ngx_cycle_t} *cycle Nginx每个worker进程对应一个ngx_cycle_t实例
 * @return {*}
 */
ngx_int_t ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    // 从参数cycle(后续调用ngx_init_cycle()函数后全局变量ngx_cycle会指向该参数)的listening数组中
    // 逐一对每个元素(ngx_listening_t结构)进行初始化，即初始化除fd字段外的其他的字段
    size_t len;
    ngx_uint_t i;
    ngx_listening_t *ls;
    socklen_t olen;
    ...
    /*
    逐一设置该数组每个元素的以下字段
        ls[i].sockaddr 套接字的IP和端口
        ls[i].addr_text_max_len 通用地址字符串长度
        ls[i].addr_text 通用地址字符串
        ls[i].backlog 监听套接字的连接数
        ls[i].rcvbuf 接收缓冲区大小
        ls[i].sndbuf 发送缓冲区大小
        ls[i].accept_filter "接收过滤器"名称(FreeBSD操作系统才有)
        ls[i].deferred_accept 是否开启TCP_DEFER_ACCEPT
    */
    ...
    for (i = 0; i < cycle->listening.nelts; i++)
    {
        ...
        if (getsockname(ls[i].fd, ls[i].sockaddr, &ls[i].socklen) == -1) //根据socket文件句柄重置ls[i].sockaddr的地址和端口
        {
        ...
        {
            ls[i].socklen = sizeof(ngx_sockaddr_t); // sockaddr长度
        }
        ...
        case AF_INET:
            ls[i].addr_text_max_len = NGX_INET_ADDRSTRLEN; // 存储ip地址的字符串 addr_text 最大长度
        ...
        olen = sizeof(int);
        // 可用于覆盖给定套接字的默认 FIB（路由表）
        // 参考链接: https://www.freebsd.org/cgi/man.cgi?query=getsockopt&sektion=2&manpath=FreeBSD+9.0-RELEASE
        ...
        // FreeBSD上叫做“接受过滤器”，而且具有多种用法。不过，在几乎所有的情况下其效果与TCP_DEFER_ACCEPT是一样的
        // 参考链接: https://www.freebsd.org/cgi/man.cgi?query=getsockopt&sektion=2&manpath=FreeBSD+9.0-RELEASE
        ...
        */
        ...
        HTTP客户端和服务端建立连接后，立即读数据（读取HTTP请求）。
            在这种情况下，client发出ACK之后，server被唤醒（accept返回），并立即试图读数据，由于client还没有发数据，
        server又再度阻塞。这对于调度是一种浪费。另外，client发的那个ACK也没有实际作用，是不必要的。

            假如server端内核忽略client发的ACK，而直接等待数据，数据收到之后再唤醒serve（accept返回），
        server醒来后就可以直接得到数据并处理。这就是TCP_DEFER_ACCEPT的作用。

            server端的socket fd是处于listen状态的，设置TCP_DEFER_ACCEPT之后，内核就会忽略ACK，
        接收到数据之后再唤醒server（accept返回）
...
void ngx_close_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++)
    {
        ...
        if (ls[i].sockaddr->sa_family == AF_UNIX && ngx_process <= NGX_PROCESS_MASTER && ngx_new_binary == 0 && (!ls[i].inherited || ngx_getppid() != ngx_parent))
        {
