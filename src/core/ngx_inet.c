...
/**
 * @description: 将二进制的地址结构，转换为文本格式
 * @param {sockaddr} *sa 通用地址结构
 * @param {socklen_t} socklen 通用地址结构长度
 * @param {u_char} *text IP和端口(IP:Port)的字符串形式
 * @param {size_t} len 地址字符串的长度
 * @param {ngx_uint_t} port 端口
 * @return {*} text长度
 */
...
ngx_inet_resolve_host(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (sin.sin_addr.s_addr == INADDR_NONE)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (ngx_inet_add_addr(pool, u, (struct sockaddr *)&sin,
                                  sizeof(struct sockaddr_in), n) != NGX_OK)
            {
                return NGX_ERROR;
            }
        }
    ...
    else
    {
        ...
        if (ngx_inet_add_addr(pool, u, (struct sockaddr *)&sin,
                              sizeof(struct sockaddr_in), 1) != NGX_OK)
        {
            return NGX_ERROR;
        }
    }
